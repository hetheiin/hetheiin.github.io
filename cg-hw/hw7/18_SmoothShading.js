...
function render() {
    ...
    let cam = vec3.fromValues(0, 0, 0);
    let cwmat = mat4.create();
    mat4.invert(cwmat, viewMatrix);
    vec3.transformMat4(cam, cam, cwmat);
    updateText(textOverlay9, `camPos = (${cam[0].toFixed(2)}, ${cam[1].toFixed(2)}, ${cam[2].toFixed(2)})`);
...
async function main() {
    try {
        ...
        textOverlay9 = setupText(canvas, "", 10);
        setupKeyboardEvents();
