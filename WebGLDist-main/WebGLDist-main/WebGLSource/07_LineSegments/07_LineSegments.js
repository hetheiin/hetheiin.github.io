...
function setupMouseEvents() {
    ...
    function handleMouseUp() {
        if (isDrawing && tempEndPoint) {
            ...
            const radius = Math.sqrt((lines[0][3]-lines[0][1])**2 + (lines[0][2] - lines[0][0])**2);
            if (lines.length === 1) {
...
function render() {
    ...
    for (let line of lines) {
        if (num == 0) { // 첫 번째 선분인 경우, yellow
            ...
            let l1 = lines[0];
            let l2 = lines[1];
            let intersections = circleLineIntersection(...l1, ...l2);

            intersections.forEach(point => {
                drawIntersections(point[0], point[1]);
            });

            if(intersections.length > 0) {
                let intersectionText = `Intersection Points: ${intersections.length} `;
                let textOverlay3 = setupText(canvas, "empty string", 3);
                ...
                let textOverlay3 = setupText(canvas, "No intersection", 3);
            }
        }
        num++;
    }
...
function circleLineIntersection(centerx, centery, destx, desty, linex1, liney1, linex2, liney2) {
    const radius = Math.sqrt((destx - centerx)**2 + (desty - centery)**2);
    let dx = linex2 - linex1;
    let dy = liney2 - liney1;
    let fx = linex1 - centerx;
    let fy = liney1 - centery;
    ...
    // t가 0~1 사이일 때만 유효한 교차점
    if (0 <= t1 && t1 <= 1) {
        intersections.push([linex1 + t1 * dx, liney1 + t1 * dy]);
    }
    if (0 <= t2 && t2 <= 1) {
        intersections.push([linex1 + t2 * dx, liney1 + t2 * dy]);
    }

    return intersections;
}

function drawIntersections(x, y) {
...
function createCircleVertex(centerX, centerY, destX, destY) {
    // arr[0] = centerX, centerY, destX, destY
    let vertices = [];
    const SEGMENTS = 100;
    const DELTA_ANGLE = 2 * Math.PI / SEGMENTS;
    const RADIUS = Math.sqrt((destX - centerX)**2 + (destY - centerY)**2);

    for(let i = 0; i <= SEGMENTS; i++) {
        let angle = i * DELTA_ANGLE;
...
function drawCircle(centerX, centerY, destX, destY) {
    let vertices = createCircleVertex(centerX, centerY, destX, destY);
